...
public class JDBCBase {
	 ...
	 * @return 结果集
	 */
	protected ResultSet query(PreparedStatement ps, Integer start,
			Integer maxCount) {
	 ...
	 */
	protected ResultSet query(PreparedStatement ps, Object[] param,
			Integer start, Integer maxCount) {
		ResultSet rs = null;
		try {
			if (param != null) {
	...
	protected int save(String sql, Object[] param) {
		...
		try {
			/**
			 * RETURN_GENERATED_KEYS：设置PreparedStatement为可返回键值的模式 一般用于获取自动生成的键值
			 */
	...
	protected int getCount(String sql) {
		...
		try {
			 ...
			 * 2.TYPE_SCROLL_INSENSITIVE,双向滚动，但不及时更新，
			 * 就是如果数据库里的数据修改过，并不在ResultSet中反应出来。
